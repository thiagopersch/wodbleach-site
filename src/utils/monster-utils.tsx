...
<monster name="${monster.name}" nameDescription="${monster.description || `a ${monster.name.toLowerCase()}`}" race="${monster.race}" experience="${monster.experience}" speed="${monster.speed}"${monster.manacost > 0 ? ` manacost="${monster.manacost}"` : ''}>
  ...
  ${
    monster.is_elements
      ? `<elements>
    ${monster.element_firePercent !== '0' ? `<element firePercent="${monster.element_firePercent}" />` : ''}
    ${monster.element_physicalPercent !== '0' ? `<element physicalPercent="${monster.element_physicalPercent}" />` : ''}
    ${monster.element_energyPercent !== '0' ? `<element energyPercent="${monster.element_energyPercent}" />` : ''}
    ${monster.element_icePercent !== '0' ? `<element icePercent="${monster.element_icePercent}" />` : ''}
    ${monster.element_holyPercent !== '0' ? `<element holyPercent="${monster.element_holyPercent}" />` : ''}
    ${monster.element_deathPercent !== '0' ? `<element deathPercent="${monster.element_deathPercent}" />` : ''}
    ${monster.element_poisonPercent !== '0' ? `<element earthPercent="${monster.element_poisonPercent}" />` : ''}
  </elements>`
      : ''
  }
  ${
    monster.is_immunities
      ? `<immunities>
    ${monster.immunity_lifedrain ? '<immunity lifedrain="1" />' : ''}
    ${monster.immunity_paralyze ? '<immunity paralyze="1" />' : ''}
    ${monster.immunity_invisible ? '<immunity invisible="1" />' : ''}
    ${monster.immunity_physical ? '<immunity physical="1" />' : ''}
    ${monster.immunity_energy ? '<immunity energy="1" />' : ''}
    ${monster.immunity_fire ? '<immunity fire="1" />' : ''}
    ${monster.immunity_poison ? '<immunity earth="1" />' : ''}
    ${monster.immunity_ice ? '<immunity ice="1" />' : ''}
    ${monster.immunity_holy ? '<immunity holy="1" />' : ''}
    ${monster.immunity_death ? '<immunity death="1" />' : ''}
    ${monster.immunity_drown ? '<immunity drown="1" />' : ''}
  </immunities>`
      : ''
  }
  ${
  ...
  ${
    monster.is_loot && monster.loot_item_id > 0
      ? `<loot>
    <item id="${monster.loot_item_id}"${monster.loot_item_countmax > 0 ? ` countmax="${monster.loot_item_countmax}"` : ''} chance="${monster.loot_item_chance}" /><!-- ${monster.loot_item_name || 'item'} -->
  </loot>`
      : ''
  }
</monster>`;
